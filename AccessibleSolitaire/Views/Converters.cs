...
namespace Sa11ytaire4All.Views
{
    ...
    public class CardStateToShowZoomCardButtonConverter : IMultiValueConverter
    {
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            if (values == null || values.Length < 3)
            ...
            var cardState = (CardState)values[1];
            ...
            var ShowZoomCardButton = (bool)values[2];

            return (isLastCardInPile && (cardState == CardState.FaceUp) && ShowZoomCardButton);
        }
    ...
    public class CardToShowZoomCardButtonConverter : IMultiValueConverter
    {
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            var cardImage = values[0] as ImageSource;
            if (cardImage == null)
            {
                return false;
            }
            ...
            var ShowZoomCardButton = (bool)values[1];

            var showZoomCardButton = (cardImage != null) && ShowZoomCardButton;
            if (showZoomCardButton)
            {
                if (cardImage != null)
                {
                    var cardImageName = cardImage.ToString();
                    if (cardImageName != null)
                    {
                        var isEmptyCard = cardImageName.Contains("empty");

                        showZoomCardButton = !isEmptyCard;
                    }
                }
            }

            return showZoomCardButton;
        }
    ...
    public class IsLastCardInPileToShowZoomCardButtonRowSpanConverter : IValueConverter
    {
        public object? Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
        {
            if (value == null)
            ...
            var isLastCardInPile = (bool)value;

            return (isLastCardInPile ? 1 : 2);
        }
